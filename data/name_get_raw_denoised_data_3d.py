...
root_path = './'
raw_data_file = osp.join(root_path, 'raw_data_test_3d', 'raw_skes_data.pkl')
save_path = osp.join(root_path, 'denoised_data_test_3d')
...
def get_one_actor_points(body_data, num_frames):
    """ Get joints for one actor (including x, y, z coordinates). """
    joints = np.zeros((num_frames, 51), dtype=np.float32)  # 17 joints with x, y, z coordinates

    start, end = body_data['interval'][0], body_data['interval'][-1]
    joints[start:end + 1] = body_data['joints'].reshape(-1, 51)  # Include x, y, z

    return joints

def get_raw_denoised_data():
    """ Get denoised joint positions (including x, y, z coordinates) from raw skeleton sequences and generate labels. """
